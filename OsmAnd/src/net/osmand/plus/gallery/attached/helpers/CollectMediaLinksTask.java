...
import static net.osmand.plus.media.MediaFileUtils.IMG_EXTENSION;
import static net.osmand.plus.media.MediaFileUtils.MPEG4_EXTENSION;
...
class CollectMediaLinksTask extends AsyncTask<Void, Void, List<Link>> {
	...
	private File copyMediaToOsmAndStorage(@NonNull PickedMedia media) {
		File destFile = MediaFileUtils.getBaseFileName(latLon.getLatitude(), latLon.getLongitude(), mediaStorageFolder, getMediaExtension(media));
